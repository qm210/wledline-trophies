...
uint16_t mode_DeadlineTrophy(void) {
    ...
    if (IS_LOGO_SEGMENT) {
        ...
        EVERY_NTH_CALL(5000) {
            DEBUG_PRINTF("[QM_DEBUG_FX] Params %.3f (%.3f %.3f) %.3f / Raw sx=%d ix=%d c1=%d c2=%d c3=%d AND AUX0: %d\n",
        ...
        for (const auto& coord : logoCoordinates()) {
            ...
            d = 255. * coord.gaussAt(center, 0.5);
            color.val = max(color.val, static_cast<uint8_t>(d));
            color.sat = min(color.sat, static_cast<uint8_t>(255. - d));

            /*
            EVERY_NTH_CALL(831) {
                DEBUG_PRINTF("[QM_DEBUG_LOGO] %.3f %.3f vs %.3f %.3f: d=%.4f made S go %d->%d and V %d->%d\n",
                    coord.uv.x, coord.uv.y, center.x, center.y, d, color.sat, color.val
                );
            }
            */
        ...
        if (SEGMENT.aux0 > 12) {
            ...
            if (fractBeat < 0.125) {
                fillLogoArray(Logo::InnerTriangle, RGBW32(200, 255, 50, 0), flashIntensity);
            } else {
                fillLogoArray(Logo::OuterTriangle, RGBW32(210, 110, 10, 0), flashIntensity);
            }
        }
    }
    ...
    if (IS_BASE_SEGMENT) {
        ...
        for (const auto& coord : baseCoordinates()) {
            ...
            /*
            // prototyped a shader and translate it now. my GLSL was:
